...
import { catalogFetch, seriesIsCurrentlyAiring, type Series } from "@/app/lib/catalog";
...
export default async function CurrentlyAiringPage() {
  ...
  const items = series.filter((title) => seriesIsCurrentlyAiring(title)).map((title) => ({ title, kind: "series" as const }));
