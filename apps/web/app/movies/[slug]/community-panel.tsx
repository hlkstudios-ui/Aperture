...
export function CommunityPanel({ movieId, authenticated, initialCommunity }: { movieId:string; authenticated:boolean; initialCommunity:Community|null }) {
  ...
  const load = async () => {
    ...
    const response = await fetch(`${apiOrigin}/community/movies/${movieId}`, { credentials:"include" });
  ...
  const rate = async (score:number) => {
    ...
    const response = await fetch(`${apiOrigin}/community/movies/${movieId}/rating`, { method:"PUT", credentials:"include", headers:{"Content-Type":"application/json"}, body:JSON.stringify({score}) });
  ...
  const submitReview = async (event:FormEvent<HTMLFormElement>) => {
    ...
    const response = await fetch(`${apiOrigin}/community/movies/${movieId}/review`, { method:"PUT", credentials:"include", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ headline:String(form.get("headline")??"" )||null, body:String(form.get("body")??""), contains_spoilers:form.get("spoilers")==="on" }) });
  ...
  const protect = async (review:Review, action:"report"|"follow"|"mute"|"block") => {
    ...
    const response = action === "report"
      ? await fetch(`${apiOrigin}/community/reports`, { method:"POST", credentials:"include", headers:{"Content-Type":"application/json"}, body:JSON.stringify({review_id:review.id,reason:"other",details:"Reported from the movie community panel."}) })
      : action === "follow" ? await fetch(`${apiOrigin}/community/follows/${review.profile_id}`, {method:"PUT",credentials:"include"})
      : await fetch(`${apiOrigin}/community/safety/${review.profile_id}/${action}`, { method:"PUT", credentials:"include" });
