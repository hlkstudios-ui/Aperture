...
This document is the input and evidence contract for moving Aperture from the verified isolated staging topology to production. The owner selected a Hostinger VPS and requested Stripe-ready billing with credentials injected last; the corresponding credential-free target is in [`deploy/production/hostinger`](../deploy/production/hostinger/README.md). Hostinger currently offers no Toronto VPS location, so the configured target is New York. It does not contain secrets or turn a staging pass into a launch approval.
...
| `API_ORIGIN`, `WEB_ORIGIN` | Public HTTPS origins; no loopback, internal service name, or shared staging hostname |
...
| `SESSION_COOKIE_DOMAIN` | Narrowest domain that permits the web/API split; verify Secure, HttpOnly, SameSite, and scope in a real browser |
...
| `MEDIA_DELIVERY_MODE`, `CDN_PUBLIC_ORIGIN`, `NEXT_PUBLIC_MEDIA_ORIGIN` | Exactly `cdn` and the approved HTTPS media hostname in production |
...
| `BILLING_PROVIDER` | A non-stub provider name. If paid subscriptions launch, its adapter, signed webhook verification, idempotency, cancellation/renewal/failure reconciliation, and checkout acceptance must be installed and tested first |
...
Application startup already rejects placeholder production credentials, HTTP public origins, missing SMTP, the development billing stub, weak monitoring tokens, and a missing error-tracking DSN. A successful startup is necessary but not sufficient evidence.

## 3. Infrastructure acceptance

The chosen provider must supply evidence for all of the following:

- Immutable web/API/worker images identified by digest; no manual production file editing.
...
- Geo-aware public ingress that strips client assertions, signs the provider-derived ISO country, fails closed when country is unavailable, and forwards both web and `/api` traffic to the direct Hostinger origin. Prove spoofed, missing, allowed, and denied countries without exposing the shared secret.
...
For Hostinger traffic rollback, retain the exact accepted API/web/backup registry digests and previous Compose release directory. Obtain dual approval for that three-digest set and migration compatibility, atomically switch only those three references, start with `--no-build`, then preserve preflight, container health, public smoke, and alert-window evidence. Application rollback never rolls back database data.
